...
**Projects.** The Place picker lists configured agent workspaces and repositories recorded with `projects.register`. Choosing one sends its project ID through `sessions.create`, so starting in a recorded repository needs `operator.write` instead of exposing a raw host path to the client. A project can run directly or supply the source for optional Worktree isolation. If its checkout was moved or removed, re-register it or run `openclaw doctor --fix` before starting another session there.
