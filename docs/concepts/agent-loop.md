...
| Agent runtime (`agents.defaults.timeoutSeconds`) | 172800s (48h)                                              | Enforced by `runEmbeddedAgent`'s abort timer. Set `0` to disable this outer run deadline. Provider liveness watchdogs remain active.                                                                                                                                                                                                                                                                                                                                                                 |
...
| Model first-event timeout                        | 120s cloud; 300s local/self-hosted                         | Bounds stream creation and the wait for the first provider event even when the outer run deadline is disabled. Recognized self-hosted provider ids keep the 300s classification when their endpoint uses an FQDN. A shorter provider, agent, or run timeout still wins.                                                                                                                                                                                                                              |
| Model stream-idle timeout                        | 120s cloud; 300s self-hosted; disabled for local endpoints | OpenClaw aborts a model request when no response chunks arrive before the idle window. Loopback/private/local endpoints may legitimately stay silent during prompt evaluation, so their implicit mid-stream watchdog is disabled while stream creation remains bounded. `models.providers.<id>.timeoutSeconds` explicitly sets the provider request and idle ceiling. Cron-triggered cloud runs with an explicit run timeout cap stream stalls at 60s so fallback candidates still have time to run. |
