...
describe("native OpenAI Responses WebSocket client integration", () => {
  ...
  it("removes HTTP-only fields injected by payload hooks before WebSocket dispatch", async () => {
    transportState.responseBatches.push([message(completedEvent("resp_1", "answer"))]);

    const result = await run(
      { messages: [userMessage("hello", 1)], tools: [] },
      { onPayload: (payload) => ({ ...payload, stream: true, background: true }) },
    );

    expect(result.stopReason).toBe("stop");
    expect(transportState.websocketRequests[0]).not.toHaveProperty("stream");
    expect(transportState.websocketRequests[0]).not.toHaveProperty("background");
  });
