...
function isWebSocketReusable(socket: ResponsesWS): boolean {
  return socket.socket.readyState === WEBSOCKET_OPEN_STATE;
}
...
function deleteOwnedWebSocketSession(cacheKey: string, entry: CachedWebSocketConnection): void {
...
function invalidateOwnedWebSocketSession(
  ...
  deleteOwnedWebSocketSession(cacheKey, entry);
}
...
function acquireWebSocket(
  ...
  if (cached) {
    ...
    if (!expired && isWebSocketReusable(cached.socket)) {
    ...
    async *[Symbol.asyncIterator]() {
      ...
      try {
        ...
        for (;;) {
          ...
          if (next.value.type === "open" && !requestDispatched) {
