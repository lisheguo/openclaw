...
describe("disposeSystemAgentSessions", () => {
  ...
  it("expires a connection-owned approval before discarding its session", async () => {
    const sessions = new Map<string, SystemAgentChatSession>([
      [
        "connection",
        sessionWithDispose(async () => {}, null, "connection:conn-1", {
          id: "approval-1",
          proposalHash: "proposal-1",
        }),
      ],
    ]);
    const expire = vi.fn((id: string, reason: string) => {
      expect(sessions.has("connection")).toBe(true);
      expect(id).toBe("approval-1");
      expect(reason).toBe("session-disposed");
      return true;
    });

    await disposeSystemAgentSessionsForOwner(sessions, "connection:conn-1", { expire } as never);
  ...
  it("expires approvals before global session disposal", async () => {
    const sessions = new Map<string, SystemAgentChatSession>([
      [
        "global",
        sessionWithDispose(async () => {}, null, "device:test", {
          id: "approval-global",
    ...
    const expire = vi.fn((id: string, reason: string) => {
      ...
      expect(id).toBe("approval-global");
      expect(reason).toBe("session-disposed");
      return true;
    });

    await disposeSystemAgentSessions(sessions, new Map(), { expire } as never);
