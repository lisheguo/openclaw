...
test("deferred cleanup re-arms when a same-id replacement starts during retirement", async () => {
  const cleanup = await startDeferredSessionCleanup("sess-provider-replacement");

  embeddedRunMock.activeIds.add("sess-provider-replacement");
  const replacementTimeout = await directSessionReq("sessions.reset", {
    key: "main",
    reason: "new",
  });
  expect(replacementTimeout.ok).toBe(false);
  cleanup.release();
  await vi.waitFor(() => {
    expect(embeddedRunMock.endWaitCalls).toEqual([
      "sess-provider-replacement",
      "sess-provider-replacement",
    ]);
  });
  expect(providerRuntimeMocks.cleanupSessionResources).not.toHaveBeenCalled();

  embeddedRunMock.activeIds.delete("sess-provider-replacement");
  embeddedRunMock.endWaiters.get("sess-provider-replacement")?.(true);
  await vi.waitFor(() => {
    expect(cleanup.retirements()).toBe(2);
